#!/usr/bin/env python3
"""
CPU Baseline Benchmark for Phase 1B.

Generates embeddings using CPU-only processing with detailed performance metrics.
"""
...
from utils.performance_logger import PerformanceLogger, format_time, format_memory
...
def main():
    ...
    # Setup logging
    log_path = log_dir / f"benchmark_cpu_{args.model}.log"
    setup_logging(log_path)

    logging.info("=" * 70)
    logging.info("Phase 1B: CPU Baseline Benchmark")
    logging.info("=" * 70)
    logging.info(f"Model: {args.model}")
    logging.info(f"Input: {args.input}")
    logging.info(f"CPU threads: {args.num_threads}")
    logging.info(f"Device: CPU")

    # Set CPU threads
    torch.set_num_threads(args.num_threads)
    logging.info(f"PyTorch using {torch.get_num_threads()} threads")

    # Initialize performance logger
    perf_logger = PerformanceLogger(f"cpu_benchmark_{args.model}")
    perf_logger.add_metadata('model', args.model)
    perf_logger.add_metadata('device', 'cpu')
    perf_logger.add_metadata('num_threads', args.num_threads)
    ...
    with perf_logger.timer("load_model"):
        try:
            ...
            # Load model (use FP32 for CPU - FP16 is inefficient on CPU)
            if is_t5:
    ...
    perf_logger.log_memory(device=None, label="after_model_load")
    ...
    with torch.no_grad():
        for batch_idx in tqdm(range(num_batches), desc=f"{args.model} [CPU]"):
            ...
            # Convert to CPU numpy (already on CPU, but keep for consistency)
            ...
            if batch_idx % 10 == 0:
                perf_logger.log_memory(device=None, label=f"batch_{batch_idx}")
    ...
    metrics_path = output_dir / f"{args.model}_cpu_1k.json"
