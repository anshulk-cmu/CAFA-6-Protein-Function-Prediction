...
def get_mean_embedding(last_hidden_state, attention_mask):
    """Calculate mean pooling over sequence length"""
...
def worker_fn(model_key, config, data_list, data_key, seed):
    ...
    logger.info(f"Model type: T5 Encoder-Decoder ({model_key})")
    ...
    for i in pbar:
        ...
        # Preprocess sequences: replace rare AAs and add spaces
...
def main(config_path):
    ...
    logger.info("Embedding generation started - T5 Encoder-Decoder models (ProtT5-XL, ProstT5)")
    ...
    logger.info("T5 encoder-decoder embedding generation finished")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CAFA-6 T5 Encoder-Decoder Embedding Generation")
